...
def normalize(signal, dtype=np.float32):
    """
    ...
    dtype : numpy dtype, optional
        Return the signal with given data type.
    ...
    Notes
    -----
    The function only supports float dtypes, since correctly normalizing
    signals of integer dtype requires a conversion to float dtype anyways.

    """
    if not np.issubdtype(dtype, np.float):
        raise TypeError('Unsupported dtype: %s, only float dtypes are '
                        'supported.' % dtype)
    # Note: np.asanyarray returns the signal's ndarray subclass
    return np.asanyarray(signal.astype(dtype) / np.max(signal))
